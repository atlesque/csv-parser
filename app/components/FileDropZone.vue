<template>
  <ClientOnly>
    <div class="w-full max-w-lg">
      <div
        class="flex flex-col items-center gap-4 rounded-xl border-2 border-dashed border-gray-300 p-10 transition-colors"
        :class="{
          'border-gray-900 bg-gray-50': isDragActive,
          'cursor-pointer hover:border-gray-400': !isParsing,
        }"
      ...
      >
        ...
        <template v-else>
          <p class="text-sm font-medium text-gray-900">{{ csvStore.file.name }}</p>
          <p class="text-xs text-gray-400">{{ formatFileSize(csvStore.file.size) }}</p>
        </template>
      </div>
      ...
      <!-- Error -->
      <div v-if="csvStore.parseError" class="mt-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
        {{ csvStore.parseError }}
      </div>
    </div>
  </ClientOnly>
